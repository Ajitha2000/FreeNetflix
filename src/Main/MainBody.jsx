...
export default function MainBody({Cate,value}){
  ...
  useEffect(() => {
    arrdata([]);
    window.scrollTo(0,0)
    axios.get("https://ajibegam.github.io/zee5apidata/free-netflix-banners.json")
    .then((res)=>{
  ...
  if(value===1){
    document.title = 'Free Netflix - All Movies - Created BY TRADEmeTRADER as Ajitha';
  }if(value===2){
    document.title = 'Free Netflix - All Web Series - Created BY TRADEmeTRADER as Ajitha';
  }if(value===3){
    document.title = 'Free Netflix - All Adult - Created BY TRADEmeTRADER as Ajitha';
  }if(value===0){
    document.title = 'Free Netflix - Watch Free Movies or WebSeries online or Download || Created BY TRADEmeTRADER as Ajitha';
  }
    return(
        <>
